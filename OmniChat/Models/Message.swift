...
final class Message {
    ...
    /// present — latency/cost/tokens/cache. The responding model itself is
    /// shown separately, right next to the "Réponse" label (see
    /// `MessageEntry.respondingModel`), so it isn't repeated here. `nil`
    /// when nothing was captured — never fabricates a value.
